...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.join(args.model_dir, 'net_wb.pth')): #and \
        ...
        logging.info("Loading model {}".format(os.path.join(args.model_dir, 'net_wb.pth')))
        net_awb.to(device=device)
        net_awb.load_state_dict(torch.load(os.path.join(args.model_dir, 'net_wb.pth'),
                                           map_location=device))
        net_awb.eval()
        # load tungsten net
...
#     #                                 device=device, s=S)
#     out_awb = deep_wb(img, task=args.task.lower(), net_awb=net_awb, net_s=net_s, net_t=net_t,
#                                     device=device, s=S)
